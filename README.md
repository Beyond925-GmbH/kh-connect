...
in-fiction intro and the days of all four trades — built on the design system
below. See [Der Flow](#der-flow-srckhpl).

**Seit dem Umbau auf vier Berufe** liegt davor ein Trichter (Helm → Fragen →
Vorschlag → Berufsliste), und der Tagesablauf ist Daten statt Konstanten.
**Alle vier** Berufe haben inzwischen einen begehbaren Tag: Dachdecker
(M1–M10 plus sieben B-Abstecher), Zimmerer (C1–C9 plus sechs Abstecher),
Zerspanung (Z1–Z9 plus sechs) und Anlagenmechanik (A1–A9 plus sechs). Siehe
[Vier Berufe](#vier-berufe-srckhplberufe).

> ⚠️ **Der Dachdecker-Tag ist aus einem Zimmerer-Tag hervorgegangen.** Der
> Ablauf trägt den Dachdecker, aber **Teile der Fachrecherche darunter sind
> noch nicht nachgezogen.** Was das konkret heißt, steht unter [Was noch
> Zimmerer ist](#was-noch-zimmerer-ist).
...
auf Deutsch —
dieselbe Sprache wie das Board und wie die Gewerke. Das Design-System darunter
(`src/components/ui/`, `src/lib/`) bleibt englisch, wie es ist.

```
berufe/              Ein Beruf ist Daten: Graph, Merkmale, Medien, Motive,
                     Copy — eine Datei je Beruf. dachdecker.ts, zimmerer.ts,
                     zerspanung.ts und anlagenmechanik.ts tragen inzwischen
                     alle vier einen Graphen; `graph: null` bleibt im Typ für
                     einen Beruf, der angekündigt, aber nicht gebaut ist.
...
glossar/             begriffe.ts trägt den gemeinsamen Bestand (20 Einträge
                     plus `Stundensatz`); daneben liegt je Beruf eine Datei
                     mit seinen gewerkseigenen Begriffen.
buehne/              Foto + StepFoto (das Motiv kommt aus `BerufDef.bilder`),
                     die Lazy-Grenzen um `three` — Dachstuhl3D (M3, M5–M8,
                     B3.2), Zuschnitt3D (M4), Beladen3D (B4.1),
                     zimmerer/Wandelement3D (C1–C7) —,
                     Dachstuhl3DFallback + kanon.ts (beide three-frei),
                     aufbauabschnitte.ts. Eigene Bühnen eines Berufs liegen
                     in seinem Unterordner.
steps/<beruf>/       Ein Modul je Step, nach Beruf getrennt. Der Text steht
                     gebündelt oben in der Datei.
```
...
einen fertigen Tag. `src/drei/` trägt die **allgemeine Szenentechnik** —
...
neue Parameter mit Default, keine geänderte Signatur. An diesen Modulen hängen
alle Bühnen; eine geänderte Signatur bricht die Tage der anderen Berufe mit.
...
1. `three` darf **nie** statisch importiert werden — auch nicht ein Hilfsexport
   ...
   Rollup ganz ins Hauptbündel, und dann liegt `three` im Erststart.
   ...
   `Zuschnitt3D`, `Beladen3D` und `zimmerer/Wandelement3D` holt ein Step nur die
   Komponente per `lazy()` und sonst ausschließlich `import type`.
...
> ⚠️ **Es sind drei Ebenen statt zwei**, und die
...
Nach dem 3D-Umbau der Dachdecker-Strecke ziehen dort nur noch M1, M2, M6, M9,
M10, B3.1, B5.1 und B9 ein Foto. Die Einträge `M3`, `M4` und `B4.1` bleiben in
der Motivliste stehen, werden aber von keinem Step mehr referenziert.
...
- **30–60 s statt 13.** Der Erststart trägt das: der Loop lädt erst 400 ms nach
  dem ersten Frame, bis dahin steht das Poster.
...
Gebaut sind **alle vier**. Jeder trägt einen vollständigen Tag: Dachdecker
(M1–M10 plus die sieben B-Abstecher), Zimmerer (C1–C9), Zerspanung (Z1–Z9) und
Anlagenmechanik (A1–A9), die drei letzten mit je sechs Abstechern.
...
Heute steht dort überall ein Graph — der Fall bleibt trotzdem im Typ, weil ein
fünfter Beruf wieder mit `null` anfängt.

Einen weiteren ergänzen heißt: `graph` mit `baueGraph` füllen (Vorbild
`dachdecker.ts`), `auftrag` schreiben, Medien nach `public/medien/media/<id>/`
legen. Die React-Module des Berufs liegen in `steps/<beruf>/`, seine eigenen
Bühnen in `buehne/<beruf>/`, und eingetragen werden sie in
`berufe/komponenten.tsx`; was dort fehlt, rendert der `Platzhalter`. **Die
Hülle ändert sich nicht.**

> ⚠️ Die **Merkmalsvektoren** fürs Matching sind gesetzt, nicht recherchiert —
> sie stammen aus der Zeit, als drei Berufe nur angekündigt waren, und sind
> beim Bau der drei Tage bewusst nicht angefasst worden (jede Verschiebung
> verschiebt die Empfehlung für alle vier). Sie reichen, um den Trichter zu
> bedienen; belegt sind sie nicht.

### Was noch Zimmerer ist

Der Dachdecker-Tag ist aus dem Zimmerer-Tag hervorgegangen und umbenannt
worden — den eigenen Zimmerer-Tag (C1–C9) gibt es erst seither daneben. Der
**Ablauf** trägt den Dachdecker (ein Dach entsteht, von der Anfrage bis zum
Feierabend), die **Fachrecherche darunter nicht überall**. Offen, nach
Dringlichkeit:

| Wo | Was | Warum es zählt |
| --- | --- | --- |
| `glossar/begriffe.ts` | Stundensatz 50–90 € stammt aus dem **Zimmerer**handwerk | Eine Zahl vor einem Vierzehnjährigen, und sie steht im Glossar-Popover. Belegen oder streichen. |
| `berufe/dachdecker.ts` | `auftrag`-Copy war für eine Zimmerei abgenommen, Betrieb ist getauscht | Abnahme ist damit offen |
| `public/medien/media/zimmerer/` | Motive zeigen Zimmerleute an Balken | Ordnername und Inhalt gehören zum Zimmerer und tragen trotzdem Karte, Hero und Szenario des Dachdecker-Tags mit |
| M5 · M7 · B3.2 | das 3D-Modell ist ein **Dachstuhl** | Tragwerk ist Zimmererarbeit; als Dachdecker-Inhalt fachlich strittig |

Erledigt ist die Karriere-Strecke: `steps/dachdecker/karrierewege.ts` trug
„rund 45.000 Euro" für den Meister — den **Zimmerer**-Wert — und führt
inzwischen eigens recherchierte Dachdecker-Zahlen samt Quelle je Abschnitt.
...
| `three`, `@react-three/fiber`, `@react-three/drei` | 3D-Bühnen: Dachstuhl in M3 (Planriss), M5–M8 und B3.2, Zuschnitt in M4, Beladen in B4.1, Wandelement in C1–C7 |

Gemessen nach `pnpm build`, gegen die gesetzten Budgets:
...
imports it. That is what keeps a WLAN dropout from stopping the app at the
stand.
...
Rot kommt nicht vor. Rot bewertet, und bewertet wird hier nicht: eine
